...
class BootstrapNavWalker extends \Walker_Nav_Menu implements BootableInterface
{
    ...
    public function url_compare($url, $rel) {
        ...
        return ((strcasecmp($url, $rel) === 0) || BootstrapNavWalker::root_relative_url($url) == $rel);
    }
    ...
    public function cssClasses($classes, $item) {
        ...
        if ($this->cpt) {
            ...
            if ($this->archive) {
                if (BootstrapNavWalker::url_compare($this->archive, $item->url)) {
    ...
    public function display_element($element, &$children_elements, $max_depth, $depth = 0, $args, &$output) {
        ...
        if ($element->is_subitem) {
            foreach ($children_elements[$element->ID] as $child) {
                if ($child->current_item_parent || BootstrapNavWalker::url_compare($this->archive, $child->url)) {
...
function nav_menu_args($args = '') {
    ...
    if (!$args['walker']) {
        $nav_menu_args['walker'] = new BootstrapNavWalker();
    }
